// commands/utility/serverbanner.js
const {
  SlashCommandBuilder,
  ContainerBuilder,
  MediaGalleryBuilder,
...
/** Build the success server banner container */
function buildServerBannerContainer(config, guild) {
  const isGif = Boolean(guild.banner?.startsWith('a_'));
  const url   = guild.bannerURL({ size: 4096, dynamic: true });
  if (!url) return null;

  const png = guild.bannerURL({ extension: 'png', size: 4096 });
  const jpg = guild.bannerURL({ extension: 'jpg', size: 4096 });
  const gif = guild.bannerURL({ extension: 'gif', size: 4096 });
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(t =>
      t.setContent(`${EMOJIS.title} **${guild.name} — Server Banner**`)
    )
    ...
    .addMediaGalleryComponents(
      new MediaGalleryBuilder().addItems(item =>
        item.setURL(url).setDescription(`Banner of ${guild.name}`)
      )
    );
...
module.exports = {
  ...
  name: 'serverbanner',
  aliases: ['sbanner', 'guildbanner', 'sb'],
  cooldown: 5,
  ...
  async execute(message, args, client, config) {
    try {
      ...
      const container = buildServerBannerContainer(config, message.guild);

      if (!container) {
        return replyThenDelete(message, {
          components: [buildStatusContainer(config, 'warn', 'This server does not have a banner.')],
          flags: MessageFlags.IsComponentsV2,
        });
      }
      ...
      console.error('[serverbanner] prefix error:', err);
      replyThenDelete(message, {
        components: [buildStatusContainer(config, 'error', 'Failed to fetch server banner.')],
        flags: MessageFlags.IsComponentsV2,
      });
    }
  },
  ...
  async slashExecute(interaction, client, config) {
    try {
      const container = buildServerBannerContainer(config, interaction.guild);
      ...
      console.error('[serverbanner] slash error:', err);
      const payload = {
        components: [buildStatusContainer(config, 'error', 'Failed to fetch server banner.')],
        flags: MessageFlags.IsComponentsV2,
      };
